				...
				if (speechManager.languages != null && speechManager.languages.Count > 1)
				{
					for (int i=0; i<speechManager.languages.Count; i++)
					{
						...
						else if (translationText.Count > (i-1))
						{
							...
							#endif
							{
								translationText [i-1] = EditField (speechManager.languages[i] + ":", translationText [i-1], true, maxWidth, "", "The display text in '" + speechManager.languages[i] + "'");
							}
						}
						else
						{
							ShowField (speechManager.languages[i] + ":", "(Not defined)", false, maxWidth);
						}
								...
								case ReferenceSpeechFiles.ByNamingConvention:
									{
										if (SeparatePlayerAudio () && speechManager.placeAudioInSubfolders)
										{
											for (int j = 0; j < KickStarter.settingsManager.players.Count; j++)
											{
												if (KickStarter.settingsManager.players[j].playerOb)
												{
													...
													{
														ShowField (" Lipsync path " + j.ToString () + ":", GetFolderName (language, true, KickStarter.settingsManager.players[j].playerOb.name), false, maxWidth);
													}
													ShowField (" Audio Path " + j.ToString () + ":", GetFolderName (language, false, KickStarter.settingsManager.players[j].playerOb.name), false, maxWidth);
												}
											}
										}
										else
										{
											...
											{
												EditorGUILayout.BeginHorizontal ();
												ShowField (" Lipsync path:", GetFolderName (language, true), false, maxWidth);
												ShowLocateButton (i, true);
												EditorGUILayout.EndHorizontal ();
											}

											EditorGUILayout.BeginHorizontal ();
											ShowField (" Audio path:", GetFolderName (language), false, maxWidth);
											ShowLocateButton (i);
											EditorGUILayout.EndHorizontal ();
										}
									}
								...
								case ReferenceSpeechFiles.ByAssetBundle:
									{
										if (speechManager.UseFileBasedLipSyncing ())
										{
											...
											{
												ShowField (" Lipsync AB:", "StreamingAssets/" + speechManager.languageLipsyncAssetBundles[i], false, maxWidth);
											}
										}

										if (!string.IsNullOrEmpty (speechManager.languageAudioAssetBundles[i]))
										{
											ShowField (" Audio AB:", "StreamingAssets/" + speechManager.languageAudioAssetBundles[i], false, maxWidth);
										}
									}
									break;
								...
								case ReferenceSpeechFiles.ByDirectReference:
									{
										if (i > 0)
										{
											...
											if (speechManager.UseFileBasedLipSyncing ())
											{
												customTranslationLipsyncFiles[i - 1] = EditField<Object> (" Lipsync file:", customTranslationLipsyncFiles[i - 1], maxWidth, apiPrefix + ".customTranslationLipsyncFiles[i-1]", "The text file to use for lipsyncing when the language is '" + KickStarter.speechManager.languages[i - 1] + "'");
											}
											customTranslationAudioClips[i - 1] = EditField<AudioClip> (" Audio clip:", customTranslationAudioClips[i - 1], maxWidth, apiPrefix + ".customTranslationAudioClips[i-1]", "The voice AudioClip to play when the language is '" + KickStarter.speechManager.languages[i - 1] + "'");
										}
										else
										{
											if (speechManager.UseFileBasedLipSyncing ())
								...
								case ReferenceSpeechFiles.ByAddressable:
									{
										if (SeparatePlayerAudio ())
										{
											for (int p = 0; p < KickStarter.settingsManager.players.Count; p++)
											{
												if (KickStarter.settingsManager.players[p].playerOb)
												{
													if (i > 0)
													{
														ShowField ("Addressable " + p.ToString () + ":", GetFilename (KickStarter.settingsManager.players[p].playerOb.name) + "_" + language, false, maxWidth);
													}
													else
													{
										...
										else
										{
											if (i > 0)
											{
												ShowField (" Addressable:", GetFilename () + "_" + language, false, maxWidth);
											}
											else
											{
							...
							case ReferenceSpeechFiles.ByAssetBundle:
								{
									if (speechManager.UseFileBasedLipSyncing ())
									{
										...
										{
											ShowField ("Lipsync AB:", "StreamingAssets/" + speechManager.languageLipsyncAssetBundles[0], false, maxWidth);
										}
									}

									if (!string.IsNullOrEmpty (speechManager.languageAudioAssetBundles[0]))
									{
										ShowField ("Audio AB:", "StreamingAssets/" + speechManager.languageAudioAssetBundles[0], false, maxWidth);
									}
								}
								break;
